...
public final class Parser {
    ...
    /**
     * Parses a single value, a list of values, or key-value pairs enclosed in parentheses.
     *
     * @return the parsed value(s) as an object.
     */
    public Object parseLiteral() {
        skipWhitespace();

        // Vérifie si la valeur est une parenthèse ouvrante
        if (position < string.length() && string.charAt(position) == '(') {
            position++; // Avancer après la parenthèse ouvrante
            skipWhitespace();

            List<Object> values = new ArrayList<>(); // Liste pour stocker les valeurs
            Map<String, String> keyValuePairs = new HashMap<>(); // Dictionnaire pour les paires clé-valeur

            while (position < string.length()) {
                skipWhitespace();

                // Gestion des chaînes entre guillemets simples
                if (string.charAt(position) == '\'') {
                    ...
                    if (position >= string.length() || string.charAt(position) != '\'') {
                        throw new ParserException("Unterminated string literal starting at position " + startPos);
                    }
                    String literal = string.substring(startPos, position++);
                    values.add(literal);
                }
                // Gestion des nombres
                else if (Character.isDigit(string.charAt(position))) {
                    int startPos = position;
                    while (position < string.length() && Character.isDigit(string.charAt(position))) {
                        position++;
                    }
                    values.add(string.substring(startPos, position));
                }

                skipWhitespace();

                // Gère les séparateurs et fin de parenthèses
                if (position < string.length() && string.charAt(position) == ',') {
                    position++; // Avancer après la virgule
                } else if (position < string.length() && string.charAt(position) == ')') {
                    position++; // Avancer après la parenthèse fermante
                    skipWhitespace();
                    break;
                } else {
                    throw new ParserException("Expected a comma or closing parenthesis at position " + position);
                }
            }

            // Retourne les paires clé-valeur ou la liste de valeurs
            return !keyValuePairs.isEmpty() ? keyValuePairs : values;
        }
        // Gestion des mots-clés spécifiques comme DEFAULT
        else if (string.startsWith("DEFAULT", position)) {
            position += "DEFAULT".length();
            return "DEFAULT";
        }
        // Gestion des littéraux simples entre guillemets
        ...
        throw new ParserException("Unexpected value at position " + position);
    }
